...
$categoryId2 = $categories[1]->term_id;

if ($categoryId == 2365) {
	$item_title = get_the_title();
	$item_jobTitle = get_post_meta($itemId, "job_title", true);

	$item_quoteSLT = get_post_meta($itemId, "quote_slt", true);
	$item_quoteSLT_cz = get_post_meta($itemId, "quote_slt_cz", true);

	$item_quoteTLT = get_post_meta($itemId, "quote_tlt", true);
	$item_quoteTLT_cz = get_post_meta($itemId, "quote_tlt_cz", true);

	$item_quoteST = get_post_meta($itemId, "quote_st", true);
	$item_quoteST_cz = get_post_meta($itemId, "quote_st_cz", true);
}


$debugLogShow = false;
$debugLog = '<br>post: '.$itemId;
$debugLog .= ', ---category:'.$categoryId;
if ($categoryId2) $debugLog .= ', ---category2:'.$categoryId2;
$debugLog .= ', ---job_title:'.$item_jobTitle;

if($debugLogShow) echo $debugLog;
...
if ( ! defined('ABSPATH')) exit;  // if direct access
	...
	if($categoryId == 2365) {
		if ($item_quoteSLT) $html_content.='<div class="element excerpt slt">'.$item_quoteSLT.'</div>';
		if ($item_quoteTLT) $html_content.='<div class="element excerpt tlt">'.$item_quoteTLT.'</div>';
		if ($item_quoteST) $html_content.='<div class="element excerpt st">'.$item_quoteST.'</div>';
		if ($item_title) $html_content.='<div class="name">'.$item_title.'</div>';
		if ($item_jobTitle) $html_content.='<div class="element job_title">'.$item_jobTitle.'</div>';
	} else {
